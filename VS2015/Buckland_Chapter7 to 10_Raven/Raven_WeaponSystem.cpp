...
void Raven_WeaponSystem::Initialize()
{
	...
	FzSet& TargetSpeed_Slow = DistToTarget.AddLeftShoulderSet("TargetSpeed_Slow", 0, 0.025, 0.150);
	FzSet& TargetSpeed_Medium = DistToTarget.AddTriangularSet("TargetSpeed_Medium", 0.025, 0.150, 0.300);
	FzSet& TargetSpeed_Fast = DistToTarget.AddRightShoulderSet("TargetSpeed_Fast", 0.150, 0.300, 1);

	FuzzyVariable& TimeVisible = m_FuzzyModule.CreateFLV("TimeVisible");
	FzSet& TimeVisible_Short = DistToTarget.AddLeftShoulderSet("TimeVisible_Short", 0, 50, 300);
	FzSet& TimeVisible_Medium = DistToTarget.AddTriangularSet("TimeVisible_Medium", 50, 300, 600);
	FzSet& TimeVisible_Long = DistToTarget.AddRightShoulderSet("TimeVisible_Long", 300, 600, 2000);

	FuzzyVariable& NoiseLevel = m_FuzzyModule.CreateFLV("NoiseLevel");
	FzSet& VeryNoisy = NoiseLevel.AddRightShoulderSet("VeryNoisy", 50, 75, 100);
	FzSet& Noisy = NoiseLevel.AddTriangularSet("Noisy", 25, 50, 75);
	FzSet& NotNoisy = NoiseLevel.AddLeftShoulderSet("NotNoisy", 0, 25, 50);

	m_FuzzyModule.AddRule(Target_Close, NotNoisy);

	m_FuzzyModule.AddRule(FzAND(Target_Medium, TargetSpeed_Slow, TimeVisible_Long), NotNoisy);
	m_FuzzyModule.AddRule(FzAND(Target_Medium, TargetSpeed_Slow, TimeVisible_Medium), NotNoisy);
	m_FuzzyModule.AddRule(FzAND(Target_Medium, TargetSpeed_Slow, TimeVisible_Short), Noisy);

	m_FuzzyModule.AddRule(FzAND(Target_Medium, TargetSpeed_Medium, TimeVisible_Long), NotNoisy);
	m_FuzzyModule.AddRule(FzAND(Target_Medium, TargetSpeed_Medium, TimeVisible_Medium), NotNoisy);
	m_FuzzyModule.AddRule(FzAND(Target_Medium, TargetSpeed_Medium, TimeVisible_Short), Noisy);

	m_FuzzyModule.AddRule(FzAND(Target_Medium, TargetSpeed_Fast, TimeVisible_Long), NotNoisy);
	m_FuzzyModule.AddRule(FzAND(Target_Medium, TargetSpeed_Fast, TimeVisible_Medium), Noisy);
	m_FuzzyModule.AddRule(FzAND(Target_Medium, TargetSpeed_Fast, TimeVisible_Short), Noisy);

	m_FuzzyModule.AddRule(FzAND(Target_Far, TargetSpeed_Slow, TimeVisible_Long), NotNoisy);
	m_FuzzyModule.AddRule(FzAND(Target_Far, TargetSpeed_Slow, TimeVisible_Medium), NotNoisy);
	m_FuzzyModule.AddRule(FzAND(Target_Far, TargetSpeed_Slow, TimeVisible_Short), Noisy);

	m_FuzzyModule.AddRule(FzAND(Target_Far, TargetSpeed_Medium, TimeVisible_Long), Noisy);
	m_FuzzyModule.AddRule(FzAND(Target_Far, TargetSpeed_Medium, TimeVisible_Medium), Noisy);
	m_FuzzyModule.AddRule(FzAND(Target_Far, TargetSpeed_Medium, TimeVisible_Short), VeryNoisy);

	m_FuzzyModule.AddRule(FzAND(Target_Far, TargetSpeed_Fast, TimeVisible_Long), Noisy);
	m_FuzzyModule.AddRule(FzAND(Target_Far, TargetSpeed_Fast, TimeVisible_Medium), VeryNoisy);
	m_FuzzyModule.AddRule(FzAND(Target_Far, TargetSpeed_Fast, TimeVisible_Short), VeryNoisy);
}
...
//
//-----------------------------------------------------------------------------
void Raven_WeaponSystem::GetNoiseLevel()
{

	//fuzzify distance and amount of ammo
	m_FuzzyModule.Fuzzify("DistToTarget", Vec2DDistance(m_pOwner->Pos(), m_pOwner->GetTargetSys()->GetTarget()->Pos()));
	m_FuzzyModule.Fuzzify("TargetSpeed", m_pOwner->GetTargetSys()->GetTarget()->Speed());
	m_FuzzyModule.Fuzzify("TimeVisible", m_pOwner->GetTargetSys()->GetTimeTargetHasBeenVisible());

	m_dAimAccuracy = m_FuzzyModule.DeFuzzify("NoiseLevel", FuzzyModule::max_av)/100;
}
...
void Raven_WeaponSystem::AddNoiseToAim(Vector2D& AimingPos)const
{
	...
	//double distance = Vec2DDistance(m_pOwner->Pos(), m_pOwner->GetTargetSys()->GetTarget()->Pos());
